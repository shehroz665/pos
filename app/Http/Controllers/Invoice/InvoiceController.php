...
class InvoiceController extends Controller
{
    ...
    //                 $q->where("updated_date","=", $searchTerm);
    //             });
    //         }
    ...
    public function sales(Request $request)
    {
        try {
            ...
            if ($request->search) {
                $searchTerm = '%' . $request->search . '%';
                $query->where(function ($q) use ($searchTerm) {
                    $q->where('updated_date', 'like', $searchTerm);
                });
            }

            $sums = [
                'total_products_sum' => $query->sum('total_products'),
                'total_price_sum' => $query->sum('total_price'),
                'total_quantity_sum' => $query->sum('total_quantity'),
                'total_cost_sum' => $query->sum('total_cost'),
            ];
            ...
            if ($request->search) {
                ...
                $query->where(function ($q) use ($searchTerm) {
                    $q->where('cust_name', 'LIKE', $searchTerm)
                        ->orWhere('cust_number', 'LIKE', $searchTerm)
                        ->orWhere('invoice_id', 'LIKE', $searchTerm)
                        ->orWhereRaw("DATE_FORMAT(updated_date, '%d-%m-%Y') LIKE ?", [$searchTerm]);
                });
            }
            $query->orderBy('invoice_id', 'desc');
            $invoices = $query->paginate($per_page);
    ...
    public function update(Request $request,$id){
        try {
            ...
            $invoice->updated_date=Carbon::now()->format('d-m-y');
            $invoice->save();
